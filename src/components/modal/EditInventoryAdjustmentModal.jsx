...
export default function EditInventoryAdjustmentModal({ open, onClose, initialData, onUpdateAdjustment }) {
    ...
    // Autofill values when modal opens or initialData changes
    useEffect(() => {
        ...
        const itemObj = {
            ...
            availableQty: isLoose ? '200kg' : '500'
        };

        setSelectedItem(itemObj);
        setItemQuery(initialData.product || '');
        setSelectedBatch(initialData.batch || 'Batch 1');

        const rawQty = initialData.qty || initialData.adjustmentQty || '1';
        const numQty = String(rawQty).replace(/[^0-9]/g, '') || '1';
        setAdjustedQuantity(numQty);

        const reason = initialData.reason || 'Sales return';
        const standardDropdownReasons = ['Sales return', 'Damaged goods', 'Expired product'];

        if (standardDropdownReasons.includes(reason)) {
            setDropdownReason(reason);
            setRadioReason(reason);
            setOtherReason('');
        } else {
            setDropdownReason('Other');
            const standardRadioReasons = ['Sales return', 'Damaged goods', 'Expired product'];
            if (standardRadioReasons.includes(reason)) {
                setRadioReason(reason);
                setOtherReason('');
            } else {
                setRadioReason('Other');
    ...
    const handleSubmit = () => {
        ...
        if (Object.keys(nextErrors).length === 0) {
            ...
            const originalQtyStr = initialData ? (initialData.qty || initialData.adjustmentQty || '') : '';
            const isNegative = String(originalQtyStr).startsWith('-');
            const unitLabel = selectedItem && selectedItem.type === 'loose' ? 'Sacks' : 'Units';
            const formattedQty = `${isNegative ? '-' : '+'}${adjustedQuantity} ${unitLabel}`;
            ...
            const updatedAdjustment = {
                ...
                channel: initialData ? (initialData.channel || 'Offline') : 'Offline',
                qty: formattedQty,
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 px-4 py-6">
            <div className="w-full max-w-[800px] rounded-2xl bg-white p-6 sm:p-8 shadow-[0_28px_80px_rgba(15,23,42,0.16)] overflow-y-auto max-h-[90vh]">
                ...
                <div className="mb-6 flex items-center justify-between">
                    <div>
                        <h2 className="text-xl font-semibold text-[#111827]">Edit inventory adjustment</h2>
                    </div>
                    <button
                        type="button"
                        onClick={onClose}
